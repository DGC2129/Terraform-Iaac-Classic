...
terraform {
  backend "s3" {
    ...
    key    = "ado-classic.tfstate"
    region = "us-east-1"
  }
}

terraform {
  required_version = "<= 2.0.11" #Forcing which version of Terraform needs to be used
  required_providers {
    aws = {
...
resource "aws_instance" "web-1" {
    ...
    tags = {
        Name = "DevOpsB23-${var.env}-Server-1"
        Owner = "Sree"
	    CostCenter = "ABCD"
        AZ = "us-east-1a"
        Tag1 = var.Megastar
...
resource "aws_instance" "web-2" {
    ...
    associate_public_ip_address = true	
    tags = {
        Name = "DevOpsB23-${var.env}-Server-2"
        Owner = "Sree"
	    CostCenter = "ABCD"
        AZ = "us-east-1b"
        Tag1 = var.Megastar
